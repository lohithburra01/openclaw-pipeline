...
def create_race_timelapse(event, year=2026, kind='R',
    ...
    for frame_idx in range(total_frames):
        ...
        title_str = f"{str(event).upper()} {year} - {session_label} - LAP {int(leader_lap)}/{total_laps}"
        cur_title_f = title_f
...
def get_google_credentials():
    """Refreshed Google OAuth credentials from the GDRIVE_* env vars."""
    ...
    for s in candidates:
        ...
        try:
            create_race_timelapse(
                event=s["event"], year=SEASON, kind=s["kind"],
...
def main():
    ...
    if args.force_session:
        ...
        create_race_timelapse(
            event=event, year=year, kind=kind, save_path=out_path,
